...
export async function calculateAndSaveAssessment(input: AssessmentRequest){
  ...
  const candidates=(lots??[])
    .filter((lot:any)=>lot.asset_accounts?.is_zakatable===true && !!lot.hawl_start_date)
    .map((lot:any)=>{
      const account=lot.asset_accounts;
      let value=new Decimal(lot.remaining_value_base||0);
      let purity=new Decimal(1);
      if(account?.asset_type==='GOLD'){
        const karat=Number(account?.metadata?.karat??24);
        purity=new Decimal(karat>0&&karat<=24?karat/24:1);
        value=new Decimal(lot.remaining_quantity).mul(goldPrice).mul(purity).mul(input.fx_rate??1);
      } else if(account?.asset_type==='SILVER'){
        const raw=Number(account?.metadata?.purity??account?.metadata?.karat??999);
        const p=raw>1?raw/1000:raw;
        purity=new Decimal(p>0&&p<=1?p:1);
        value=new Decimal(lot.remaining_quantity).mul(silverPrice).mul(purity).mul(input.fx_rate??1);
      }
  ...
  const snapshot={assessmentDate:input.assessment_date,valuationDate,methodCode:method.code,methodVersion:method.version,calendarType:method.calendar_type,nisabStandard:standard,nisabValue:nisabValue.toString(),prices:{gold:goldPrice,silver:silverPrice},fxRate:input.fx_rate??1,candidateLots:candidates.length,calculatedAt:new Date().toISOString()};
