...
export async function listAssets(){
  ...
  const {data:lines,error:lineError}=await supabase.from('zakat_assessment_lines').select('lot_id,quantity,market_value,eligible_value,zakat_amount,eligibility_status,valuation_price,valuation_currency,fx_rate,explanation,lots!inner(asset_account_id,hawl_start_date,hawl_due_date)').eq('assessment_id',latest.id);
