...
export async function getDashboard(){
 ...
 const [{data:assets,error:ae},{data:assessments,error:xe}]=await Promise.all([
  supabase.from('asset_accounts').select('id,name,asset_type,is_zakatable,currency,metadata').eq('user_id',user.id).eq('status','ACTIVE'),
