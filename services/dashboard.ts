...
export async function getDashboard(){
 ...
 const [{data:assets,error:ae},{data:assessments,error:xe}]=await Promise.all([
  ...
  supabase.from('zakat_assessments').select('id,zakat_due,total_zakatable_value,status,assessment_date,currency,calculation_snapshot').eq('user_id',user.id).order('assessment_date',{ascending:false}).limit(10)
 ]);
 ...
 // Dashboard Hawl figures are monitoring indicators only. The assessment engine uses lot.hawl_start_date + the selected calendar/method.
 ...
 // IMPORTANT: Zakat due comes only from a saved assessment snapshot, never from asset metadata or a 2.5% UI estimate.
 const assessedDue=n(latest?.zakat_due||0);
 const assessedBase=n(latest?.total_zakatable_value||0);
 const assetDetails=rows.map((a:any)=>{const cur=current(a),cost=purchase(a),gain=cur.sub(cost),age=days(a?.metadata?.purchase_date);return{id:a.id,name:a.name,type:a.asset_type,currency:a.currency,currentSar:cur.toFixed(2),purchaseSar:cost.toFixed(2),gainSar:gain.toFixed(2),gainPct:cost.gt(0)?gain.div(cost).mul(100).toFixed(2):'0.00',zakatable:!!a.is_zakatable,purchaseDate:a?.metadata?.purchase_date||'',hawlDays:age,hawlCompleted:age>=354,zakahDueSar:null,zakahPaidSar:null,zakahStatus:'ASSESSMENT_REQUIRED',weightKg:a?.metadata?.weight_kg??''}});
 return {assetCount:rows.length,totalCurrentSar:totalCurrent.toFixed(2),totalPurchaseSar:totalPurchase.toFixed(2),gainSar:totalCurrent.sub(totalPurchase).toFixed(2),gainPct:totalPurchase.gt(0)?totalCurrent.sub(totalPurchase).div(totalPurchase).mul(100).toFixed(2):'0.00',zakatableValueSar:zakatableValue.toFixed(2),hawlCompleted:hawlCompleted.length,hawlValueSar:hawlValue.toFixed(2),upcoming30,goldWeightKg:goldWeightKg.toFixed(3),explicitZakahDueSar:assessedDue.toFixed(2),assessedZakatableValueSar:assessedBase.toFixed(2),latestAssessmentDate:latest?.assessment_date??null,latestAssessmentStatus:latest?.status??null,latestZakat:latest?.zakat_due??'0',latestCurrency:latest?.currency??'SAR',latestAssessments:assessments??[],assetDetails};
}
