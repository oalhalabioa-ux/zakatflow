...
export async function getDashboard(){
 ...
 const [{data:assets,error:ae},{data:assessments,error:xe},{data:allocations,error:pe},{data:payments,error:pye}]=await Promise.all([
 ...
 if(ae)throw ae;if(xe)throw xe;if(pe)throw pe;if(pye)throw pye;
 ...
 const validAssessmentIds=new Set((assessments??[]).map((x:any)=>x.id)),paidByAsset=new Map<string,Decimal>();let allocatedPaid=new Decimal(0);
 for(const p of allocations??[]){if(!validAssessmentIds.has((p as any).assessment_id))continue;const amount=n((p as any).allocated_amount);allocatedPaid=allocatedPaid.add(amount);const aid=(p as any).asset_account_id;if(aid)paidByAsset.set(aid,(paidByAsset.get(aid)||new Decimal(0)).add(amount))}
