...
export const vatProfileSchema=z.object({organization_id:z.string().uuid(),tax_registration_number:z.string().trim().max(30).optional().nullable(),registration_status:z.enum(['NOT_REGISTERED','REGISTERED','PENDING','DEREGISTERED']),registration_date:z.string().date().optional().nullable(),filing_frequency:z.enum(['MONTHLY','QUARTERLY']),standard_rate:z.coerce.number().refine(rate=>rate===15,{message:'SAUDI_STANDARD_VAT_RATE_MUST_BE_15'}),period_start_month:z.coerce.number().int().min(1).max(12)}).superRefine((p,ctx)=>{if(p.registration_status==='REGISTERED'&&!p.tax_registration_number?.trim())ctx.addIssue({code:'custom',message:'VAT_REGISTRATION_NUMBER_REQUIRED',path:['tax_registration_number']})});
export const vatDocumentSchema=z.object({organization_id:z.string().uuid(),document_type:z.enum(['SALES','PURCHASE']),document_kind:z.enum(['INVOICE','CREDIT_NOTE']),document_number:z.string().trim().min(1).max(80),transaction_date:z.string().date(),counterparty_name:z.string().trim().min(1).max(160),counterparty_tax_number:z.string().trim().max(30).optional().nullable(),supply_type:z.enum(['STANDARD','ZERO_RATED','EXEMPT','OUT_OF_SCOPE']),net_amount:z.coerce.number().finite().nonnegative(),recoverable_percent:z.coerce.number().min(0).max(100).default(100),notes:z.string().trim().max(1000).optional().nullable()}).superRefine((p,ctx)=>{if(p.document_type==='SALES'&&p.recoverable_percent!==100)ctx.addIssue({code:'custom',message:'VAT_RECOVERY_SALES_ONLY',path:['recoverable_percent']})});
